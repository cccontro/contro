...
fun main(args: Array<String>) {
    ...
    aBoxList.forEach { name ->
        ...
        autoAddNS(ont, turtle)
        writeIfChanged(ont, file, "ttl", turtle)

        autoAddNS(infOnt, turtle)
...
fun autoAddNS(
    ont: OWLOntology,
    format: TurtleDocumentFormat
) {
    // Collect all entities from ontology
    val allEntities = ont.getSignature(EXCLUDED)

    for (entity in allEntities) {
        val rawBase = entity.iri.namespace
        val slash = rawBase.lastIndexOf('/')
        val hash = rawBase.lastIndexOf('#')
        val base = rawBase.substring(0, maxOf(slash, hash) + 1)

        // Skip if already known
        if (format.getPrefixIRI(IRI.create(base)) != null) continue
